...
import static com.MythiCode.ocrkit.Orientation.getSupportedRotation;
...
public class CameraView1 implements SurfaceHolder.Callback, CameraViewInterface {
    ...
    public void initCamera(RelativeLayout relativeLayout, boolean hasTextReader, char flashMode
            , boolean isFillScale, boolean isTakePictureMode, boolean isScanningText) {
        ...
        int height = convertDeviceHeightToSupportedAspectRatio(relativeLayout.getWidth(), relativeLayout.getHeight());
...
//                            cameraViews.peek().showFrame();
    ...
    private void updateCameraSize() {
        try {
            ...
            final int y = convertDeviceHeightToSupportedAspectRatio(relativeLayout.getWidth(), relativeLayout.getHeight());
            if (mCamera == null) return;
            List<Camera.Size> supportedPreviewSizes = mCamera.getParameters().getSupportedPreviewSizes();
            List<Camera.Size> supportedPictureSizes = mCamera.getParameters().getSupportedPictureSizes();
            optimalSize = getOptimalPreviewSize(supportedPreviewSizes, relativeLayout.getWidth(), y);
            final Camera.Size optimalPictureSize = getOptimalPreviewSize(supportedPictureSizes, relativeLayout.getWidth(), y);
